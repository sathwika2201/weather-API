...
function Login(){
    ...
    function loginSuccess(res)
    {
        ...
        {
            var T1=document.getElementById('T1');
            setSession("sid", T1.value, (24 * 0.5));
    ...
    function register(){
        ...
        if(RT3.value==="" || RT3.value.length !== 10 || isNaN(RT3.value))
        {
            RT3.style.border = "1px solid red";
            RT3.focus();
            return;
        }
        if(RT4.value==="" || !RT4.value.endsWith("@gmail.com"))
        {
            RT4.style.border = "1px solid red";
            RT4.focus();
            return;
        }
        if(RT5.value==="" || !/(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*\W)/.test(RT5.value))
        {
    ...
    return(
        <div className='full-height'>
            <div id='header' className='loginheader'><center> weather forecast application </center></div>
